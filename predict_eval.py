...
from dataset_polygon import char2idx, idx2char, encode_text
import Levenshtein
...
def greedy_decode(model, image_tensor):
    ...
    with torch.no_grad():
        ...
        for _ in range(MAX_LEN):
            ...
            out = model.decoder(ys, memory, tgt_mask=model.generate_square_subsequent_mask(ys.size(1)).to(DEVICE))
            ...
            if hasattr(model.decoder.transformer_decoder.layers[-1], 'self_attn'):
                ...
                attn_weights = model.decoder.transformer_decoder.layers[-1].self_attn.attn_weights
...
def evaluate_dataset(image_dir, label_dir, num_samples=None, save_prefix="test"):
    ...
    for img_file in tqdm(image_files):
        ...
        with open(label_path, encoding='utf-8') as f:
            for line in f:
                ...
                if len(parts) < 9 or parts[8] == "###": continue
    ...
    avg_metrics = {k: v / all_metrics['total'] for k, v in all_metrics.items() if k != 'total'}
...
test_results, test_metrics = evaluate_dataset(IMAGE_DIR, LABEL_DIR, num_samples=50, save_prefix="test")
...
if os.path.exists(UNSEEN_IMAGE_DIR):
    ...
    for metric in test_metrics:
        print(f"{metric}: {test_metrics[metric]:.4f} (test) vs {unseen_metrics[metric]:.4f} (unseen)")
    print("==============================\n")
